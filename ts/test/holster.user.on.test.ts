...
describe("holster.user.on", () => {
  ...
  test("on for not-yet-existing child key, two updates - two events", (t, done) => {
    ...
    // has ever existed, so the underlying wire listener has to stay on the
    // parent (watching for child to become a rel later) rather than
    // switching straight onto an already-existing rel's target at
    // subscribe time.
