...
// Unlike every other holster.on.*.test.ts (all single-instance, writer and
...
describe("holster.on nested", () => {
  ...
  test("setup", async () => {
    await new Promise<void>((resolve, reject) => {
      serverUser.create("host", "password", (err?: string | null) => {
        if (err && !err.includes("already exists")) reject(new Error(err))
        else resolve()
      })
    })
    await new Promise<void>((resolve, reject) => {
      serverUser.auth("host", "password", (err?: string | null) => {
  ...
  test("sees a value-only update to an already-known object-valued child", async () => {
    await new Promise<void>(resolve => {
      serverUser
        ...
        .next("nested1")
        .next("a")
        .put({ct: "1"}, resolve as never)
    })

    const events: Record<string, {ct: string}>[] = []
    const cb = (data: unknown) =>
      events.push(data as Record<string, {ct: string}>)
    await new Promise<void>(resolve => {
      clientUser
        .get([host, "shared"])
        .next("nested1")
