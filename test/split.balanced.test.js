...
// ~opt.size/2 mark (halfKey) rather than at the overflow tail, so both halves
// end up near opt.size/2 instead of the original file staying full with a
// tiny new file containing only the overflow entry.
//
// With size:200 and entries ~55 bytes each (same value length as
// split.hybrid.test.js), four souls fit across three entries (165 < 200) but
// not four (220 > 200), and two entries exceed opt.size/2 (110 > 100) so
...
describe("balanced split", () => {
  ...
  const now = 1000
  // 36 chars — same value as split.hybrid.test.js; ~55 bytes per encoded entry.
  const long = "this value is long enough to trigger"

  // Write four souls A–D. The overflow fires when D is added (A+B+C+D > 200).
  // halfKey was recorded at C (when write.text = A+B > 100 = opt.size/2), so
  // the split point is "C": "!" keeps A+B and new file "C" receives C+D.
  ...
  // file "C" has ~110 bytes after the split, well under opt.size:200, so it
  // absorbs the new soul without splitting again.
