...
import Store, {fileSystem} from "../src/store.js"
...
describe("radisk concurrency", () => {
  const dataDir = "test/radisk.concurrency-data"
  const saveTestDir = "test/radisk.concurrency-save-data"
  after(() => {
  ...
  const store = Store({file: dataDir, size: 2000})
  ...
  test("every concurrent write survives - none are silently lost or left partial", async () => {
    ...
    const oneRound = async () => {
      ...
      await new Promise((resolve, reject) =>
        store.put(graph, err => (err ? reject(new Error(err)) : resolve())),
      )

      const data = await new Promise(resolve =>
        store.get({"#": soul}, (err, ack) => resolve(ack && ack[soul])),
      )

      if (!data || data.name !== "x" || data.type !== "file") {
        failures++
  ...
  test("radisk.save's own completion callback fires exactly once per thrash cycle", async () => {
    ...
    // radisk.save itself.
    const saveOpt = {file: saveTestDir, size: 2000}
    saveOpt.store = fileSystem(saveOpt)
    const radisk = Radisk(saveOpt)

    const extraCallCounts = []
    ...
    await Promise.all(
      keys.map(
        (key, i) => new Promise(resolve => radisk(key, `value${i}`, resolve)),
      ),
    )
